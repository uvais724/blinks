# --- Stage 1: Build stage ---
    ...
    # Copy package.json first (for Docker caching)
    COPY package*.json ./
    ...
    # Install minimal required libraries for Chrome to run
    RUN apt-get update && apt-get install -y \
        fonts-liberation \
        libappindicator3-1 \
        libasound2 \
    ...
    # Copy only the built app and installed node_modules from the builder
