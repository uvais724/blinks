    ...
    <div v-if="showSharePopup" class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white p-6 rounded-lg shadow-lg w-96">
        ...
        <div class="mb-4">
          ...
          <input v-model="searchQuery" type="text" placeholder="Search user" class="input input-bordered w-full"
            @input="searchUsers" />
          ...
          <div class="flex flex-wrap gap-2 mt-2">
            <div v-for="user in selectedUsers" :key="user._id"
              ...
              {{ user.username }}
              <button class="ml-2 text-red-500 hover:text-red-700" @click="removeUser(user._id)">
                &times;
              </button>
            </div>
          </div>
          ...
          <ul class="mt-2 max-h-40 overflow-y-auto">
            <li v-for="user in filteredUsers" :key="user._id" class="p-2 hover:bg-gray-100 cursor-pointer"
              @click="addUser(user)">
              {{ user.username }}
            </li>
          </ul>
...
const searchUsers = async () => {
  if (searchQuery.value.length < 3) {
    filteredUsers.value = []; // Clear results if query is less than 3 characters
    return;
  }
